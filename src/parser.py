...
class SketchOutputParser: # parses a line of Sketch's output 
	...
	def p_assertexpr(self, p):
		...
		if expr_lhs in self.stmt_map:
			...
			self.to_be_deleted_stmts.add("%s = %s" % (expr_lhs, expr_rhs)) # delete this and use the given lhs instead of the tmp_var
		...
		if p[5] in self.stmt_map:
			self.to_be_deleted_stmts.add("%s = %s" % (p[5], self.stmt_map[p[5]])) # delete, since this is the input expression
	...
	def run_parser(self, inputfile, to_be_deleted_stmts_init):
		...
		while not l.startswith("}"):
			...
			if l.isspace():
				...
				self.three_addr_code.append(l.rstrip().replace(";", "")) # assignment statements before assert will be in three-addr code (TODO: check)
