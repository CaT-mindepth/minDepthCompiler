...
class DependencyGraph:
  def __init__(self, filename, state_vars, var_types):
    ...
    self.read_write_flanks = {s: {} for s in
                              self.state_variables}  # key: state variable, value: {"read":read_flank, "write":write flank}
    ...
    for scc in nx.strongly_connected_components(self.dep_graph):
      ...
      for v in scc:
        for e in self.dep_graph.edges([v]):
          if (e[0] in scc) and (e[1] in scc) and (
              e not in self.read_write_edges):  # both nodes in scc, not a rw flank edge
            scc_edges.append(e)
    ...
    for u1, v1 in self.dep_graph.edges:
      ...
      if ":" in u.rhs and ":" in v.rhs:  # both u and v are phi nodes
        ...
        while i < len(u.lhs) and i < len(v.lhs):
          ...
          elif (not u.lhs[i].isdigit()) or (
          not v.lhs[i].isdigit()):  # mismatched character is not a digit, so not the same variable
            same_var = False
